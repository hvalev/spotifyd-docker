...
FROM --platform=$BUILDPLATFORM rust:1.79.0-buster AS rust_fix
...
###
FROM rust:1.79.0-buster AS alsa-build
RUN apt-get -y update && \
...
###
FROM debian:bookworm-20240701-slim AS alsa-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20240701-slim AS alsa-dbus-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20240701-slim AS pulseaudio-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20240701-slim AS pulseaudio-dbus-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20240701-slim AS portaudio-release
RUN apt-get update && \
    apt-get install -yqq --no-install-recommends libasound2 && \
    rm -rf /var/lib/apt/lists/* && \
    groupadd -r spotify && \
...
###
FROM debian:bookworm-20240701-slim AS portaudio-dbus-release
RUN apt-get update && \
