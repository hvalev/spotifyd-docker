...
FROM --platform=$BUILDPLATFORM rust:1.87.0-bookworm AS rust_fix
...
###
FROM rust:1.87.0-bookworm AS alsa-build
RUN apt-get -y update && \
...
###
FROM debian:bookworm-20250520-slim AS alsa-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20250520-slim AS alsa-dbus-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20250520-slim AS pulseaudio-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20250520-slim AS pulseaudio-dbus-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20250520-slim AS portaudio-release
RUN apt-get update && \
    apt-get install -yqq --no-install-recommends ca-certificates libasound2 && \
    rm -rf /var/lib/apt/lists/* && \
    groupadd -r spotify && \
...
###
FROM debian:bookworm-20250520-slim AS portaudio-dbus-release
RUN apt-get update && \
