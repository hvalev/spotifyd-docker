...
###
FROM debian:trixie-20251117-slim AS alsa-release
RUN apt-get update && \
...
###
FROM debian:trixie-20251117-slim AS alsa-dbus-release
RUN apt-get update && \
...
###
FROM debian:trixie-20251117-slim AS pulseaudio-release
RUN apt-get update && \
...
###
FROM debian:trixie-20251117-slim AS pulseaudio-dbus-release
RUN apt-get update && \
...
###
FROM debian:trixie-20251117-slim AS portaudio-release
RUN apt-get update && \
    apt-get install -yqq --no-install-recommends ca-certificates libasound2t64 libpulse0 && \
    rm -rf /var/lib/apt/lists/* && \
    groupadd -r spotify && \
...
###
FROM debian:trixie-20251117-slim AS portaudio-dbus-release
RUN apt-get update && \
