...
###
FROM debian:bookworm-20241016-slim AS alsa-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20241016-slim AS alsa-dbus-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20241016-slim AS pulseaudio-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20241016-slim AS pulseaudio-dbus-release
RUN apt-get update && \
...
###
FROM debian:bookworm-20241016-slim AS portaudio-release
RUN apt-get update && \
    apt-get install -yqq --no-install-recommends libasound2 && \
    rm -rf /var/lib/apt/lists/* && \
    groupadd -r spotify && \
...
###
FROM debian:bookworm-20241016-slim AS portaudio-dbus-release
RUN apt-get update && \
